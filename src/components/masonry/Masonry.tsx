...
const Masonry: React.FC<MasonryProps> = ({
  ...
  useEffect(() => {
    // Only preload if items actually have an img property and no custom renderItem is provided
    if (!renderItem && items.some(i => i.img)) {
      ...
      preloadImages(items.map(i => i.img as string)).then(() => setImagesReady(true));
    } else {
      setImagesReady(true);
    }
  }, [items, renderItem]);
  ...
  const grid = useMemo<GridItem[]>(() => {
    ...
    return items.map(child => {
      ...
      const height = child.height; // use actual height instead of / 2
  ...
  // Adjust container height to match the tallest column
  ...
  useLayoutEffect(() => {
    ...
    // Set container height explicitly so elements below it don't overlap
  ...
  }, [grid, imagesReady, stagger, animateFrom, blurToFocus, duration, ease, containerHeight]);
