...
import { X, BookOpen, FileText, ExternalLink, Sparkles } from 'lucide-react'
		...
		<DialogModal open onClose={onClose} className='max-w-4xl w-full max-h-[90vh] overflow-y-auto p-0 md:p-0 relative flex flex-col shadow-2xl bg-white dark:bg-slate-950 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-800 rounded-xl custom-scrollbar'>
			...
			<div className='sticky top-0 z-50 flex items-center justify-between px-6 py-4 bg-white/95 dark:bg-slate-950/95 backdrop-blur-lg border-b border-slate-200 dark:border-slate-800'>
				<div className='flex items-center gap-2'>
					<span className="text-blue-400 text-lg font-mono font-bold">&lt;{book.name}/&gt;</span>
				</div>
						...
						<div className="space-y-4 pt-2">
							...
							<div className="space-y-3 text-sm">
								<div className="flex flex-col gap-1">
									<span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5"><span className="text-base">✍️</span> Author</span>
									<span className="font-medium text-slate-900 dark:text-slate-100">{book.author || '未知'}</span>
								</div>

								{book.status && (
									<div className="flex flex-col gap-1">
										<span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5"><span className="text-base">🎯</span> Status</span>
										<span className="font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded w-fit">
											{book.status === 'finished' ? '已读完' : book.status === 'reading' ? '正在阅读' : '打算看'}
										</span>
									</div>
								)}
								...
								{book.readDate && (
									<div className="flex flex-col gap-1">
										<span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5"><span className="text-base">🗓️</span> Read Date</span>
										<span className="font-mono font-medium text-slate-700 dark:text-slate-300">{book.readDate}</span>
									</div>
								)}
								...
								{book.rating && (
									<div className="flex flex-col gap-1">
										...
										<span className="font-mono font-bold text-amber-500">{book.rating} / 5.0</span>
									</div>
								)}
							</div>
						</div>
						...
						<div className="flex flex-col gap-2 mt-6">
							{book.doubanUrl && (
								<button
									...
									className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-lg font-bold transition-all hover:scale-[1.02] active:scale-[0.98] shadow-md text-sm"
								>
							...
							{book.epubUrl && (
								<button
									...
									className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold transition-all hover:scale-[1.02] active:scale-[0.98] shadow-md text-sm"
								>
							...
							{book.pdfUrl && (
								<button
									...
									className="w-full flex items-center justify-center gap-2 py-2.5 px-4 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold transition-all hover:scale-[1.02] active:scale-[0.98] shadow-md text-sm"
								>
